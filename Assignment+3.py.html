...
def answer_one():
    ...
    energy.head()
    energy['Energy Supply'][energy['Energy Supply'] == '...'] = None
    energy['Energy Supply'] = energy['Energy Supply'].apply(lambda x: x*1000000 if(np.all(pd.notnull(x))) else x)
...
def answer_seven():
    ...
    x = Top15.sort_values(by= 'ratio', ascending=False).iloc[0]
    return (x.name, x['ratio'])
answer_seven()
...
def answer_eight():
    ...
    x = Top15.sort_values(by= 'pop', ascending=False).iloc[2]
    return x.name
answer_eight()
...
df2.columns = ['Country', 'Continent']
df2.set_index('Country',inplace=True)
df2
Top15 = answer_one()
Top15['PopulationEst'] = Top15['Energy Supply'] / Top15['Energy Supply per Capita']
x = pd.merge(df2, Top15, left_index=True, right_index=True)
df3 = x.groupby('Continent').agg(['size', 'sum','mean','std'])
df3.columns
